...
def problem_group_update(request):
    ...
    description = description if description != None else problem_group.description
    if len(description) > 200 :
        return E_DESCRIPTION_FORMAT
...
def problem_create(request):
    ...
    Problem.objects.create(problem_group=problem_group, index=index, type=type, content=content, ans_count=ans_count, answer=answer,
        field1=field[0], field2=field[1], field3=field[2], field4=field[3], field5=field[4], field6=field[5], field7=field[6],
        creater=user)
...
def problem_adjust_order(request):
    ...
    if newindex < index :
        Problem.objects.filter(problem_group=problem_group, index__gte=newindex, index__lt=index).update(index=F('index') + 1)
    elif newindex > index :
        Problem.objects.filter(problem_group=problem_group, index__gt=index, index__lte=newindex).update(index=F('index') - 1)
