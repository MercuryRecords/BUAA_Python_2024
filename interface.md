...
|------------|-------|------|-----|----------|---------------|
| username          | 管理员用户名  | 是    | 字符串 | "john"             | 长度不多于 100 个字符 |
| group_name | 用户群组名称 | 是    | 字符串 | "group1" | 长度不多于 100 个字符 |

- 返回信息：

| 状态码 | 信息      | 数据                                  | 备注 |
|-----|---------|-------------------------------------|----|
| 200 | 用户群组删除成功 | {"code": 200, "message": "用户群组删除成功"} | -  |
| 401 | 用户群组不存在  | {"code": 401, "message": "用户群组不存在"}  | -  |
...
| 状态码 | 信息            | 数据                                        | 备注 |
|-----|---------------|-------------------------------------------|----|
| 200 | 用户加入用户群组成功     | {"code": 200, "message": "用户加入用户群组成功"}     | -  |
| 401 | 用户或用户群组不存在     | {"code": 401, "message": "用户或用户群组不存在"}     | -  |
...
| 状态码 | 信息            | 数据                                        | 备注 |
|-----|---------------|-------------------------------------------|----|
| 200 | 用户退出用户群组成功     | {"code": 200, "message": "用户退出用户群组成功"}     | -  |
| 401 | 用户或用户群组不存在     | {"code": 401, "message": "用户或用户群组不存在"}     | -  |
...
| username        | 管理员用户名  | 是    | 字符串 | "john"                  | 长度不多于 100 个字符 |
| group_name      | 用户群组名称   | 是    | 字符串 | "group1"                | 长度不多于 100 个字符 |
| new_description | 新的用户群组描述 | 是    | 字符串 | "new group description" | 长度不多于 255 个字符 |

- 返回信息：

| 状态码 | 信息        | 数据                                    | 备注 |
|-----|-----------|---------------------------------------|----|
| 200 | 用户群组信息修改成功 | {"code": 200, "message": "用户群组信息修改成功"} | -  |
| 401 | 用户群组不存在    | {"code": 401, "message": "用户群组不存在"}    | -  |
...
| 状态码 | 信息             | 数据                                                                                                                                                                                                                                                                                                                                                        | 备注 |
|-----|----------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----|
| 200 | 用户群组列表获取成功      | {"code": 200, "message": "用户群组列表获取成功", "data": [{"name": "group1", "description": "group1 description", "creator": "creator1", "create_time": "2022-01-01 10:00:00", "members": ["user1", "user2"]}, {"name": "group2", "description": "group2 description", "creator": "creator2", "create_time": "2022-01-02 10:00:00", "members": ["user3", "user4"]}]} | -  |
| 401 | 用户群组列表获取失败，页码越界 | {"code": 401, "message": "用户群组列表获取失败，页码越界"}                                                                                                                                                                                                                                                                                                                | -  |
...
| to_search          | 搜索关键字           | 否                 | 字符串 | "Math"    | 搜索题单标题和描述中的关键字                               |
...
- 请求方式：POST
- 请求URL：/api/admin_get_sensitive_word_num
- 请求参数：
...
- 返回信息：{"code": 200, "message": "敏感词数量获取成功", "data": 敏感词数量}

## 管理员获取敏感词列表

- 请求方式：POST
- 请求URL：/api/admin_get_sensitive_word_list
- 请求参数：

| 参数名             | 参数说明      | 是否必须 | 类型  | 例子        | 备注   |
|-----------------|-----------|------|-----|-----------|------|
| username        | 管理员用户名    | 是    | 字符串 | "JohnDoe" |      |
| to_search       | 搜索关键字     | 否    | 字符串 | "原神"      |
| page            | 页码        | 否    | 整数  | 1         | 从1开始 |
| number_per_page | 每页容纳的题单数 | 否    | 整数  | 10        |      |

- 返回信息：{"code": 200, "message": "敏感词列表获取成功", "data": 敏感词列表}
- 错误信息：...
