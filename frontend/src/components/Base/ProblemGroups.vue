...
const fetchProblems = async () => {
  try {
    const response = await API.post('/admin_get_problem_groups', {
...
const updateProblemGroupFunc = async () => {
  try {
    const response = await API.post('/admin_problem_group_update', updateProblemGroup.value,
        {
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded'
          }
...
const deleteProblemGroup = async (problem: any) => {
  try {
    ...
    const response = await API.post('/admin_problem_group_delete', {
      username: data.username,
      problem_group_id: problem.id
    }, {
      headers: {
      ...
      <el-table :data="showProblems" style="width: 100%" :default-sort="{ prop: 'id', order: 'ascending' }">
        ...
        <el-table-column label="名称" width="300">
          <template #default="scope">
            <router-link
                :to="{ name: 'adminSheet', query: { username:data.username,sheetId: scope.row.id } }"
                class="problem-link">
              {{ scope.row.title }}
            </router-link>
          </template>
